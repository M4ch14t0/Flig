...
/**
 * Script para Popular Fila no Railway - Usuário Existente
 * 
 * Este script conecta ao banco Railway e popula uma fila para o usuário testeestab@email.com
 */
...
async function populateExistingUserQueue() {
  let connection;
  
  try {
    console.log('🚀 POPULANDO FILA PARA USUÁRIO EXISTENTE NO RAILWAY');
    console.log('==================================================');
    ...
    // Buscar o estabelecimento específico
    console.log('\n🔍 Buscando estabelecimento testeestab@email.com...');
    ...
    if (establishments.length === 0) {
      console.log('❌ Estabelecimento testeestab@email.com não encontrado');
      console.log('🔍 Verificando estabelecimentos existentes...');
      
      const [allEstablishments] = await connection.execute(
        'SELECT id, nome_empresa, email_empresa, status FROM estabelecimentos LIMIT 10'
      );
      
      console.log('\n📋 Estabelecimentos encontrados:');
      allEstablishments.forEach(est => {
        console.log(`- ${est.nome_empresa} (${est.email_empresa}) - Status: ${est.status}`);
      });
    ...
    // Criar fila de teste para este estabelecimento
    console.log('\n📋 Criando fila de teste...');
    const queueId = randomUUID();
    const queueData = {
      nome: 'Fila Teste - testeestab@email.com',
      estabelecimento_id: establishment.id,
      descricao: 'Fila criada para teste do usuário testeestab@email.com',
      status: 'ativa',
      max_avancos: 5,
      valor_avancos: 2.00,
      tempo_estimado: 5
    ...
    console.log('\n🎯 PRÓXIMOS PASSOS');
    console.log('==================');
    console.log('1. Faça login com: testeestab@email.com / Abcd1234');
