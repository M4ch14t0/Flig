-- Schema MySQL CORRIGIDO para Sistema Flig
-- Este arquivo contém todas as tabelas necessárias para o sistema de filas virtuais
-- 
-- CORREÇÕES APLICADAS:
-- 1. Adicionada tabela 'notificacoes' faltante
-- 2. Adicionada tabela 'user_devices' faltante  
-- 3. Corrigido nome da tabela 'transacoes_pagamentos' (plural)
-- 4. Adicionados campos faltantes
-- 
-- @author Flig Team
-- @version 1.0.1
...
-- Tabela de transações de pagamento (CORRIGIDO: plural)
...
-- TABELA FALTANTE: Dispositivos de usuários (para push notifications)
...
-- Criação de views para relatórios (CORRIGIDAS)
CREATE OR REPLACE VIEW view_estatisticas_estabelecimentos AS
SELECT 
    e.id,
    e.nome_empresa,
    e.cnpj,
...
CREATE OR REPLACE VIEW view_estatisticas_filas AS
SELECT 
    f.id,
    f.nome,
...
-- Triggers para atualização automática de estatísticas (CORRIGIDOS)
DELIMITER //
...
SELECT 'Schema corrigido criado com sucesso!' as status;
...
SELECT table_name as 'Tabelas Criadas' FROM information_schema.tables 
