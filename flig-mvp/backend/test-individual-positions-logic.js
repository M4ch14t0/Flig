/**
 * Script de Teste para Lógica de Fila com Posições Individuais
 * 
 * Este script testa a nova arquitetura onde cada posição é tratada individualmente:
 * - Cada posição é um objeto independente que pode conter múltiplos clientes
 * - Subposições são reorganizadas apenas dentro da mesma posição
 * - Após uma chamada, apenas reorganizamos as subposições dentro de cada posição
 * 
...
async function testQueueLogic() {
  try {
    console.log('🚀 Iniciando teste da lógica de fila com posições individuais');
    
    // Conectar ao Redis
    await redisService.connectRedis();
    
    const queueId = 'test-queue-individual-positions';
    ...
    if (allStepsCorrect) {
      ...
      console.log('✅ A lógica de fila com posições individuais está funcionando perfeitamente!');
    } else {
